...
class ReturnKeyJumpFilter(QObject):
    ...
    def eventFilter(self, obj, event):
        if event.type() == QEvent.KeyPress:
            ...
            # 处理方向键 - 移动并进入编辑
                ...
                # 如果正在编辑，不处理方向键（让用户可以在编辑器内移动光标）
                if self.table.state() == self.table.EditingState:
                    return False

                # 使用定时器延迟处理，确保默认的方向键移动先执行
                QTimer.singleShot(0, self._handle_arrow_key_navigation)
                return False  # 不拦截，让默认处理先执行
    ...
    def _handle_arrow_key_navigation(self):
        """处理方向键导航后进入编辑模式"""
        try:
            current = self.table.currentIndex()
            if current.isValid():
                row, col = current.row(), current.column()
                self._enter_edit_mode_if_editable(row, col)
        except Exception as e:
            print(f"方向键导航处理失败: {e}")
