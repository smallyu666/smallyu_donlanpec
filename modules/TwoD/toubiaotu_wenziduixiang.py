...
from modules.TwoD.TwoD_peizhi import get_project_save_dir, get_autocad_instance, _find_open_doc, wait_cad_idle, \
    refresh_doc, get_current_doc, ensure_dwg_path, must_exist_file, open_drawing_with_wait, safe_modify, \
...
def twoDgeneration(product_id):
    try:
        ...
        path1 = "BEU投标图"  # 你的业务名/前缀
        template_path = os.path.abspath("BEU投标图.dwg")
        ...
        # === 写入 Word 指定 handle ===
        safe_modify(doc, "829fe", text)
        print(f"✅ 写入 handle 829fe: {text}")
        ...
        for idx in range(1, 4):  # 可按需要改范围
            ...
            if param_map.get(t_key, "").lower() == pipe_mat_type.lower() and \
                ...
                break
        safe_modify(doc, "829fe", flange_mat_grade)
        # 从 元件计算结果表 里取
        cursor.execute("""
            SELECT Name, Value
            FROM 产品设计活动表_元件计算结果表
        ...
        for idx in range(1, 4):  # 可按需要改范围
            ...
            if param_map.get(t_key, "").lower() == pipe_mat_type.lower() and \
                ...
                flange_mat_grade = param_map.get(f"接管法兰材料牌号{idx}", "").strip()
                break
        safe_modify(doc, "82a06", flange_mat_grade)
