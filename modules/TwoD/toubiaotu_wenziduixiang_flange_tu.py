...
def twoDgeneration(product_id, flange):
    ...
    if acad is None:
        ...
        return
    dwg_path = os.path.abspath("法兰凸.dwg")
    ...
    if regulation_text:
        safe_modify(doc,"1375", regulation_text)
        # modify_by_handle(doc,"12EE", regulation_text)

    regulation_text = get_flang_jisuan_value(product_id, "法兰名义内径", flange)
    if regulation_text:
        safe_modify(doc,"1374", regulation_text)
        # modify_by_handle(doc,"130A", regulation_text)
    ...
    if regulation_text:
        safe_modify(doc,"1378", regulation_text)
        # modify_by_handle(doc,"1382", regulation_text)

    regulation_text = get_flang_jisuan_value(product_id, "法兰名义内径", flange)
    regulation_text2 = get_flang_jisuan_value(product_id, "法兰颈部大端名义厚度", flange)
    regulation_text = float(regulation_text)+2*float(regulation_text2)
    if regulation_text:
        safe_modify(doc,"1384", regulation_text)
        # modify_by_handle(doc,"1381", regulation_text)

    regulation_text = get_flang_jisuan_value(product_id, "法兰名义厚度", flange)
    if regulation_text:
        safe_modify(doc,"1379", regulation_text)
        # modify_by_handle(doc,"1317", regulation_text)

    regulation_text = get_flang_jisuan_value(product_id, "法兰颈部高度", flange)
    if regulation_text:
        safe_modify(doc,"137A", regulation_text)
        # modify_by_handle(doc,"1385", regulation_text)

    regulation_text = get_flang_jisuan_value(product_id, "法兰总高", flange)
    if regulation_text:
        safe_modify(doc,"137B", regulation_text)
    ...
    # modify_by_handle(doc, "1314", regulation_text)

    regulation_text = get_flang_jisuan_value(product_id, "螺栓数量", flange)
    if regulation_text:
        safe_modify(doc,"137D", regulation_text)

    regulation_text = get_flang_jisuan_value(product_id, "螺栓孔直径", flange)
    if regulation_text:
        safe_modify(doc,"137E", round(float(regulation_text)))
    regulation_text = get_flang_jisuan_value(product_id, "法兰直边段高度", flange)
    if regulation_text:
        safe_modify(doc,"1389", regulation_text)
        # modify_by_handle(doc,"1386", regulation_text)

    regulation_text = get_flang_jisuan_value(product_id, "螺栓中心圆直径", flange)
    if regulation_text:
        safe_modify(doc,"1376", regulation_text)
        # modify_by_handle(doc, "12EF", regulation_text)

    safe_modify(doc, "16DE", flange)
    safe_modify(doc, "172C", flange)
    ...
    #     # === 修改 CAD 文字 ===
    #     modify_by_handle(doc, "1318", groove_depth)
