...
from modules.TwoD.TwoD_peizhi import ensure_dwg_path, must_exist_file, open_drawing_with_wait, modify_text_by_handle
...
def choose_cad_exe():
    ...
    exe_path = filedialog.askopenfilename(
        ...
        filetypes=[("AutoCAD 可执行文件", "acad.exe"), ("所有文件", "*.*")]
    )
...
def try_start_autocad_com():
    try:
        ...
        acad.Visible = True
        print("✅ 通过 COM 成功连接 AutoCAD")
        return acad
    except Exception as e:
        print(f"⚠️ COM 启动失败: {e}")
        return None
...
def get_autocad_instance():
    ...
    return start_autocad_by_path(exe_path)
