...
from modules.TwoD.TwoD_peizhi import get_project_save_dir, get_autocad_instance, wait_cad_idle, refresh_doc, \
    _find_open_doc, get_current_doc, open_drawing_with_wait, ensure_dwg_path, must_exist_file, safe_modify, \
...
def twoDgeneration(product_id):
    try:
        ...
        if acad is None:
            ...
            return
        path1 = "AES投标图"
        template_path = os.path.abspath("AES投标图-6-2.dwg")
        ...
        for idx in range(1, 4):  # 可按需要改范围
            ...
            if param_map.get(t_key, "").lower() == pipe_mat_type.lower() and \
                ...
                break
        safe_modify(doc, "85c0c", flange_mat_grade)
        # 从 元件计算结果表 里取
        cursor.execute("""
            SELECT Name, Value
            FROM 产品设计活动表_元件计算结果表
        ...
        for idx in range(1, 4):  # 可按需要改范围
            ...
            if param_map.get(t_key, "").lower() == pipe_mat_type.lower() and \
                ...
                flange_mat_grade = param_map.get(f"接管法兰材料牌号{idx}", "").strip()
                break
        safe_modify(doc, "85c14", flange_mat_grade)
