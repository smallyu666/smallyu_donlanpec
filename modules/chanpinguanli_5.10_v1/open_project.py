...
def open_project():
    ...
    try:
        ...
        # 0509新修改--打开项目时双根检测（提示文案在 project_path_relocate.maybe_warn_duplicate_project_root_on_open）
        from modules.chanpinguanli import project_path_relocate

        if not project_path_relocate.maybe_warn_duplicate_project_root_on_open(
            bianl.main_window, project_id, project_info, folder_path
        ):
            print("[open_project] 用户取消打开（重复项目根提示）")
            return
