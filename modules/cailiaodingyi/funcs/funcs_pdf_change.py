...
# 浮头法兰：螺栓孔边缘至密封槽最小距离（配置库 user_config）
FLOATING_HEAD_HMIN_CONFIG_ID = "2.8.4.2"
FLOATING_HEAD_HMIN_PARAM_NAME = "螺栓孔边缘至密封槽最小距离"
FLOATING_HEAD_HMIN_LEGACY_DEFAULT = "5"
FLOATING_HEAD_FLANGE_ELEMENT_NAME = "浮头法兰"
...
def is_none_template_name(template_name) -> bool:
    """模板名为空或字面量 None 时视为 None 模板。"""
    s = str(template_name or "").strip()
    return (not s) or s.lower() == "none"


def insert_or_update_element_para_data(product_id, element_para_info, template_name=None):
    """根据产品ID判断是否更新数据，如果存在模板名称不同则删除原记录并插入新数据"""
    connection = get_connection(**db_config_1)
    try:
        with connection.cursor() as cursor:
            ...
            # 浮头法兰 Hmin：按配置库 user_config(2.8.4.2) 一次解析
            # （数值型：None 模板不带入配置默认值，保留材料库原值）
            ...
            for item in element_para_info:
                ...
                # 浮头法兰：螺栓孔边缘至密封槽最小距离 ← user_config(2.8.4.2)（非 None 模板）
                if (
...
def load_element_data_by_product_id(product_id, only_visible=True):
    """
    根据产品ID从产品活动库中读取已更新的元件信息（用于刷新左侧表格）。
    默认 only_visible=True，与结构树「是否显示」一致，避免确定/清空等刷新把隐藏元件重新画出来。
    """
...
def resolve_floating_head_hmin_default_from_user_config() -> str:
    """
    浮头法兰「螺栓孔边缘至密封槽最小距离」默认值。
    读配置库 user_config(id=2.8.4.2)；读不到时回退为 5。
    """
    global _FLOATING_HEAD_HMIN_FROM_CONFIG_CACHE
    if _FLOATING_HEAD_HMIN_FROM_CONFIG_CACHE is not None:
        return _FLOATING_HEAD_HMIN_FROM_CONFIG_CACHE
    val = _get_user_config_value(FLOATING_HEAD_HMIN_CONFIG_ID)
    if val is not None and str(val).strip():
        _FLOATING_HEAD_HMIN_FROM_CONFIG_CACHE = str(val).strip()
    else:
        _FLOATING_HEAD_HMIN_FROM_CONFIG_CACHE = FLOATING_HEAD_HMIN_LEGACY_DEFAULT
    return _FLOATING_HEAD_HMIN_FROM_CONFIG_CACHE
