...
# 新增 -- 在本地浏览器中打开
    ...
    def refresh_all_tabs_readonly_state(self):
        ...
        # 项目管理页对整页使用了 readonly=False，会把所有 QLineEdit 设为可编辑，
        # 从而覆盖「查看模式」下项目信息区应只读的状态（例如从条件输入返回时）。
        try:
            pm = getattr(bianl, "project_mode", "view")
            if pm in ("new", "edit"):
                common_usage.set_project_inputs_editable(True)
            else:
                common_usage.set_project_inputs_editable(False)
        except Exception as _e:
            print(f"[refresh_all_tabs_readonly_state] 恢复项目信息只读状态失败: {_e}")
